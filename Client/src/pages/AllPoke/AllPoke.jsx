...
const AllPoke = () => {
    ...
    const handleSearch = async () => {
        ...
        const description = await fetch(`https://pokeapi.co/api/v2/pokemon-species/${searchInput.toLowerCase()}`);
  ...
  return (
    <div
      ...
      style={{backgroundImage: `url(${imgUrl})`,}}>
        <section>
        <input className="hero__glow-cta"
            type="text"
            placeholder="Enter Pokemon name"
            value={searchInput}
            onChange={(e) => setSearchInput(e.target.value)}
        />
        <button className="hero__glow-cta" onClick={handleSearch}>Search</button>
        <div>
            {searchResult && (
                <div>
             ...
             <ul>
                {details.map(pokemon => (
                    ...
                    {pokemon.name} - {pokemon.stats[0].stat.name}: {pokemon.stats[0].base_stat}
